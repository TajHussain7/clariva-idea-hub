...
import { LayoutDashboard, FileText, BarChart2, Settings, Plus, Search, Bell, MoreVertical, Activity, Database, Smartphone, Globe, Code2 } from 'lucide-react';

export function Dashboard() {
  const ideas = [
    { id: 1, title: 'AI Developer Assistant', domain: 'DevTools', complexity: 'High', score: 88, status: 'Analyzed', icon: Code2, color: 'text-indigo-400', bg: 'bg-indigo-500/10' },
    { id: 2, title: 'Micro-SaaS Analytics', domain: 'B2B SaaS', complexity: 'Medium', score: 74, status: 'Analyzed', icon: BarChart2, color: 'text-cyan-400', bg: 'bg-cyan-500/10' },
    { id: 3, title: 'Local Event Finder', domain: 'Consumer', complexity: 'Low', score: 45, status: 'Analyzed', icon: Globe, color: 'text-amber-400', bg: 'bg-amber-500/10' },
    { id: 4, title: 'Automated CRM Sync', domain: 'B2B SaaS', complexity: 'High', score: 0, status: 'Processing', icon: Database, color: 'text-emerald-400', bg: 'bg-emerald-500/10' },
    { id: 5, title: 'Habit Tracker Pro', domain: 'Mobile App', complexity: 'Low', score: 62, status: 'Analyzed', icon: Smartphone, color: 'text-rose-400', bg: 'bg-rose-500/10' },
    { id: 6, title: 'Workflow Automation', domain: 'Productivity', complexity: 'Medium', score: 0, status: 'Draft', icon: Activity, color: 'text-slate-400', bg: 'bg-slate-500/10' },
  ];
    ...
    <div className="min-h-screen bg-[#0B1120] text-white flex font-sans selection:bg-indigo-500/30">
      ...
      <div className="w-64 bg-[#080E1C] border-r border-white/5 flex flex-col shrink-0">
        ...
        <nav className="flex-1 px-4 py-6 space-y-1">
          <a href="#" className="flex items-center gap-3 px-3 py-2 rounded-lg bg-indigo-500/10 text-indigo-400 font-medium border-l-2 border-indigo-500 transition-colors">
            <LayoutDashboard className="w-4 h-4" />
            Dashboard
          </a>
          <a href="#" className="flex items-center gap-3 px-3 py-2 rounded-lg text-white/60 hover:text-white hover:bg-white/5 border-l-2 border-transparent transition-colors">
        ...
        <header className="h-20 px-8 flex items-center justify-between border-b border-white/5 bg-[#0B1120]/80 backdrop-blur-md sticky top-0 z-10">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden md:flex items-center gap-2 bg-white/5 border border-white/10 rounded-full px-4 py-2">
              ...
              <input type="text" placeholder="Search ideas..." className="bg-transparent border-none outline-none text-sm text-white placeholder:text-white/40 w-48" />
            </div>
        ...
        <main className="flex-1 overflow-y-auto p-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
            ...
            <div className="bg-white/5 border border-white/10 rounded-2xl p-5 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-2xl font-heading font-bold">74<span className="text-lg text-white/40 font-normal">%</span></p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex items-center gap-2">
              <button className="text-sm text-white/60 hover:text-white px-3 py-1 rounded-md transition-colors">View All</button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {ideas.map((idea) => (
              <div key={idea.id} className="bg-[#0D1426] border border-white/10 rounded-2xl p-5 hover:border-indigo-500/50 transition-colors group flex flex-col h-full relative overflow-hidden">
                {idea.score > 80 && (
                ...
                <div className="flex justify-between items-start mb-4 relative z-10">
                  <div className={`w-10 h-10 rounded-xl \${idea.bg} \${idea.color} flex items-center justify-center`}>
                ...
                <h3 className="font-heading font-semibold text-lg mb-2 relative z-10">{idea.title}</h3>
                
                <div className="flex items-center gap-2 mb-6 text-xs relative z-10">
                  <span className="bg-white/5 px-2 py-1 rounded-md text-white/70 border border-white/5">{idea.domain}</span>
                  <span className="w-1 h-1 rounded-full bg-white/20"></span>
                  <span className="text-white/50">{idea.complexity} Complexity</span>
                </div>
                  ...
                  {idea.status === 'Analyzed' ? (
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 radar-placeholder relative flex items-center justify-center">
                        ...
                        <span className={`relative z-10 font-bold \${idea.score > 80 ? 'text-indigo-400' : idea.score > 60 ? 'text-cyan-400' : 'text-white'}`}>
                          {idea.score}
                        </span>
                      </div>
                      <div className="text-sm">
                        <p className="text-white/50 text-xs">Overall Score</p>
                        <p className="font-medium text-white/90">Good Potential</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 w-full">
                      ...
                      <span className="text-xs text-white/40 font-mono">65%</span>
                    </div>
                  ) : (
                    <p className="text-sm text-white/40 italic">Awaiting details...</p>
                  )}
                </div>
              </div>
            ))}
